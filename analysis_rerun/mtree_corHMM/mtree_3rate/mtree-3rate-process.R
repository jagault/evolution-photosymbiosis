...
setwd(here("analysis_rerun/mtree_corHMM/mtree_3rate/"))

# Define path to results, will be used to read in files in for-loop below
path <- c("~/Dropbox/osg_results/stree/mtree/mtree_3rate")
...
for (i in 1:length(t)){
  files <- list.files(path = paste(path, "/3-rate_", i, sep = ""), 
